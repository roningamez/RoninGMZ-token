...
describe("Contract", function () {
	...
	describe("Set Variables", function() {
		...
		it("Should get contractCreateTime", async function() {
			res = await getVar(Token.contractCreateTime,
				'Token.contractCreateTime', 10);
		});

		it("Should get contractLaunchTime as 0", async function() {
			res = await getVar(Token.contractLaunchTime,
				'Token.contractLaunchTime', 10);
		});
		it("Should launch the contract", async function() {
			await Token.launchContract();
		});
		it("Should get contractLaunchTime", async function() {
			res = await getVar(Token.contractLaunchTime,
				'Token.contractLaunchTime', 10);
		});
		...
		it("Should set sellEnabled", async function() {
			res = await setgetVar(
				Token.setSellEnabled,
				'Token.setSellEnabled',
				Token.sellEnabled,
				'Token.sellEnabled',
				true
