...
describe("Contract", function () {
	...
	describe("Get the total Fees", function() {
		it("Current Fees", async function() {
			fees += Number(await Token.liquidityFee());
			fees += Number(await Token.marketingFee());
	...
	describe("Transfers", function() {
		const n = 27500000000;
		nf = 0;
		it("Should set the fee: " + n, async function () {
			nf = n - n*fees/100;
			console.log(n + " " + fees + " " + nf);
		});
		it("Should transfer 1 -> 2: " + n, async function () {
			...
			expect(Number(bal1_) - Number(n)).to.equal(bal1);
			expect(Number(bal2_) + Number(nf)).to.equal(bal2);
		});
		it("Should transfer 3 -> 4: " + n, async function () {
			...
			expect(Number(bal3_) - Number(n)).to.equal(bal3);
			expect(Number(bal4_) + Number(nf)).to.equal(bal4);
		});
	});
